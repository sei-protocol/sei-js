...
`parseArgs()` now returns a frozen `AppConfig` snapshot, and every transport handles requests against that snapshot. Stopping one runtime releases only its provider, while other runtimes keep their original signer. HTTP startup also checks both its compatibility option and the snapshot that controls signing, so conflicting programmatic options fail closed before listening.
