...
`parseArgs()` now returns a frozen `AppConfig` snapshot. HTTP transports close their `serverFactory` and request/session handling over that snapshot, stdio `connect` / `onmessage` stay on the same snapshot after another runtime's `stop()` drops the process-wide wallet provider memo, and both HTTP transports keep wallet tools hidden after a subsequent private-key `initializeConfig()`. Direct wallet-on-HTTP startup still exits 1.
