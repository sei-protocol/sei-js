...
describe('AssetList Tests', () => {
	it('should have the correct structure for each network', () => {
		...
		networks.forEach((network) => {
			expect(Array.isArray(AssetListJSON[network])).toBeTruthy();
			AssetListJSON[network].forEach((asset) => {
				...
				asset.denom_units.forEach((denomUnit: DenomUnit) => {
...
it('should contain the "sei" asset with correct properties in each network', () => {
	Object.keys(AssetListJSON).forEach((network) => {
		const seiAsset = AssetListJSON[network as Network].find((asset) => asset.symbol === 'SEI');
