...
export class Service {
	static Simulate(req: SimulateRequest, initReq?: fm.InitReq): Promise<SimulateResponse> {
		return fm.fetchReq<SimulateRequest, SimulateResponse>(`/cosmos/tx/v1beta1/simulate`, {
			...initReq,
			method: "POST",
	...
	static GetTx(req: GetTxRequest, initReq?: fm.InitReq): Promise<GetTxResponse> {
		return fm.fetchReq<GetTxRequest, GetTxResponse>(`/cosmos/tx/v1beta1/txs/${req["hash"]}?${fm.renderURLSearchParams(req, ["hash"])}`, {
			...initReq,
			method: "GET"
		});
	}
	...
	static GetBlockWithTxs(req: GetBlockWithTxsRequest, initReq?: fm.InitReq): Promise<GetBlockWithTxsResponse> {
		return fm.fetchReq<GetBlockWithTxsRequest, GetBlockWithTxsResponse>(
			`/cosmos/tx/v1beta1/txs/block/${req["height"]}?${fm.renderURLSearchParams(req, ["height"])}`,
			{ ...initReq, method: "GET" }
		);
	}
}
