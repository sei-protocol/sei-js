...
export class Msg {
	static GrantAllowance(req: MsgGrantAllowance, initReq?: fm.InitReq): Promise<MsgGrantAllowanceResponse> {
		return fm.fetchReq<MsgGrantAllowance, MsgGrantAllowanceResponse>(`/cosmos.feegrant.v1beta1.Msg/GrantAllowance`, {
			...initReq,
			method: "POST",
	...
	static RevokeAllowance(req: MsgRevokeAllowance, initReq?: fm.InitReq): Promise<MsgRevokeAllowanceResponse> {
		return fm.fetchReq<MsgRevokeAllowance, MsgRevokeAllowanceResponse>(`/cosmos.feegrant.v1beta1.Msg/RevokeAllowance`, {
			...initReq,
			method: "POST",
			body: JSON.stringify(req, fm.replacer)
		});
