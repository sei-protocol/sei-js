...
export class Msg {
	static SubmitProposal(req: MsgSubmitProposal, initReq?: fm.InitReq): Promise<MsgSubmitProposalResponse> {
		return fm.fetchReq<MsgSubmitProposal, MsgSubmitProposalResponse>(`/cosmos.gov.v1beta1.Msg/SubmitProposal`, {
			...initReq,
			method: "POST",
	...
	static VoteWeighted(req: MsgVoteWeighted, initReq?: fm.InitReq): Promise<MsgVoteWeightedResponse> {
		return fm.fetchReq<MsgVoteWeighted, MsgVoteWeightedResponse>(`/cosmos.gov.v1beta1.Msg/VoteWeighted`, {
			...initReq,
			method: "POST",
			body: JSON.stringify(req, fm.replacer)
		});
	...
	static Deposit(req: MsgDeposit, initReq?: fm.InitReq): Promise<MsgDepositResponse> {
		return fm.fetchReq<MsgDeposit, MsgDepositResponse>(`/cosmos.gov.v1beta1.Msg/Deposit`, {
			...initReq,
			method: "POST",
			body: JSON.stringify(req, fm.replacer)
		});
