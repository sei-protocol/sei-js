...
const WalletConnectButton = ({ buttonStyles }: WalletConnectButtonProps) => {
	...
	const renderButton = () => {
		if (!connectedWallet) {
			return (
				<button style={styles.button} onClick={() => setShowConnectModal(true)}>
					connect wallet
				</button>
			);
		}
		...
		return (
			<div style={styles.wrapper}>
				<button style={{ ...styles.button, ...buttonStyles?.button }} onClick={() => setShowMenu(true)}>
					{accountLabel}
				</button>
				{showMenu && (
						...
						<div style={{ ...styles.menu, ...buttonStyles?.menu }}>
							{accounts && (
								<p onClick={copyAddress}>
									<span style={{ ...styles.menuItem, ...buttonStyles?.menuItem }}>{recentlyCopied ? 'copied' : 'copy address'}</span>
								</p>
							)}
							<p style={{ ...styles.menuItem, ...buttonStyles?.menuItem }} onClick={changeWallet}>
								change wallet
							</p>
							<p onClick={disconnect} style={{ ...styles.menuItem, ...buttonStyles?.menuItem }}>
								disconnect
							</p>
						</div>
					</OutsideClickHandler>
	...
	return (
		<>
			{renderButton()}
			{showConnectModal && <WalletSelectModal walletSelectStyles={buttonStyles?.walletSelectStyles} setShowConnectModal={setShowConnectModal} />}
		</>
	);
};
