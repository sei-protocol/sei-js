...
export function decodeValidatorInfo(
  ...
  return {
    pubkey: data.pub_key && decodePubkey(assertObject(data.pub_key)),
    votingPower: apiToBigInt(assertNotEmpty(data.voting_power)),
    address: data.address
      ? fromHex(assertNotEmpty(data.address))
      : new Uint8Array(),
...
function decodeTxProof(data: RpcTxProof): responses.TxProof | undefined {
  if (!data.data) {
    return;
  }
  return {
