...
export class SeiSigningCosmWasmClient extends SigningCosmWasmClient {
  ...
  public static async connectWithSigner(
    ...
    signer: OfflineSigner,
    options: SigningCosmWasmClientOptions = {}
  ): Promise<SeiSigningCosmWasmClient> {
    const tmClient = await Tendermint35Client.connect(endpoint);
    return new SeiSigningCosmWasmClient(tmClient, signer, options);
  }
