...
describe('HTTP wallet isolation across later starts', () => {
	...
	it('still terminates a direct private-key HTTP start before listen', async () => {
		...
		const streamable = new StreamableHttpTransport({ port: 8080, host: HOST, path: PATH, appConfig }, { listenFactory });
		const sse = new HttpSseTransport({ port: 8080, host: HOST, path: PATH, appConfig }, { listenFactory });
	...
	it.each(['http-sse', 'streamable-http'] as const)('rejects a wallet-enabled %s appConfig before listen', async (mode) => {
		consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
		const processExit = jest.spyOn(process, 'exit').mockImplementation((code) => {
			throw new Error(`process.exit called with code ${code}`);
		});
